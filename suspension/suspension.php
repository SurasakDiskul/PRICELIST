...
include('../php/connect.php');
    ...
    $ss = $_GET['id'];
    $sql = "SELECT * FROM  `tbsuspension` WHERE id = '$ss'";
    $res = mysqli_query($conn, $sql);
    $row = mysqli_fetch_assoc($res);

    $aa = $row['types'];
    $sql1 = "SELECT * FROM  `tbimgsus` WHERE types = '$aa'";
    ...
    <div class="row">
      <div class="col-lg-12 text-center">
          <ul class="list-inline mb-0">
           ...
           <h1 class="text-lg text-white mt-2"><?php echo $row['nameeng'] ?></h1>
      </div>
    </div>
  </div>
</section>
...
<section class="section course bg-gray">
    ...
    <?php while ($row1 = mysqli_fetch_assoc($res1)) : ?>
					<img src="../imgsus/<?php echo $row1['img'] ?>" alt="" class="img-fluid">
          <?php endwhile ?>
			</div>
